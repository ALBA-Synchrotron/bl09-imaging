        ...
        if os.path.isdir(specific_folder) and (('tomo' in folder) or ('TOMO' in folder)):
            ...
            if tomo_txrm_file!='None':
                if flatfield_txrm_file=='None':
                    order_tomo_FF='s'       
                    call_txrm2nexus=txrm2nexus_program_name+' '+tomo_txrm_file+' '+'-o='+order_tomo_FF
                    os.system(call_txrm2nexus)
                else:
                    order_tomo_FF='sb'       
                    call_txrm2nexus=txrm2nexus_program_name+' '+tomo_txrm_file+' '+flatfield_txrm_file+' '+'-o='+order_tomo_FF   
