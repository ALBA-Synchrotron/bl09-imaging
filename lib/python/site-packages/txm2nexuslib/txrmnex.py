    ...
    def convert_metadata(self):
        ...
        if ole.exists('ImageInfo/XrayMagnification'):   
            ...
            if (self.magnification != 0.0):
                pass
            elif (self.magnification == 0.0 and self.pixelsize != 0.0): # magnification in micrometers
        ...
        #### Energy(enc) for each image calculated from Energyenc ####
        try:
            if (energyenc_name is not None):
                if ole.exists('PositionInfo/MotorPositions'):   
                    stream = ole.openstream('PositionInfo/MotorPositions')
                    number_of_floats= self.num_axis*self.nSampleFrames
                    struct_fmt = '<'+str(number_of_floats)+'f'
                    ...
                    if verbose: print "ImageInfo/Energyenc: \n ",  energies     
                    self.nxinstrument['source']['energyenc'] = nxs.NXfield(
                       name = 'energyenc', value=energenc, attrs = {'units': 'eV'})
                    self.nxinstrument['source']['energyenc'].write()
        except:
            pass   
        ####
