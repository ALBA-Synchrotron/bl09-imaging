...
class XradiaFile(object):
    ...
    detecorenc_name = property(get_detectorenc_name)
    ...
    def get_end_date(self):
        enddate = self.dates[self.nSampleFrames-1]
        [endday, endhour] = enddate.split(" ")
        [endmonth, endday, endyear] = endday.split("/")
        [endhour, endminute, endsecond] = endhour.split(":")
...
class xrmNXtomo(object):
    ...
    def __init__(self, reader, hdf5_file_name, program_name, program_version,
                 program_args, title='X-ray tomography',
                 sourcename='ALBA', sourcetype='Synchrotron X-ray Source', 
                 sourceprobe='x-ray', instrument='BL09 @ ALBA', 
                 sample='Unknown'):
        self.reader = reader
    ...
    def convert_metadata(self):
        self.nxentry = nxs.NXentry(name=self.definiton)
        ...
        self.nxentry['program_name'].attrs['configuration'] = \
                    ('%s %s' % (self.programname, ' '.join(self.program_args))

        # Sample-ID
        sample_name = self.rader.get_sample_id()
        ...
        pixel_size = self.reader.pixel_size
        self.nxinstrument['sample']['x_pixel_size'] = nxs.NXfield(
                                                name='x_pixel_size',
                                                value=pixel_size,
                                                attrs = {'units': 'um'})
        ...
        self.nxsample['y_translation'] = nxs.NXfield(
                 name='y_translation', value=xpositions, attrs={'units': 'um'})
        # Z sample translation: nxsample['z_translation']
