...
class mosaicnex:
    ...
    def __init__(self, files, files_order='s', title='X-ray tomography', 
        ...
        self.num_input_files_verify = len(self.orderlist) #number of 's' 'b' and 'd'.
    ...
    def convert_mosaic(self): 
        ...
        if not self.brightexists:
            print('\nWarning: Bright-Field is not present, normalization will not be possible if you do not insert a Bright-Field (FF). \n') 
        ...
        if (self.index_FF_file != -1):
            ...
            for i in range(0, self.nSampleFramesFF):
                ...
                if self.datatypeFF == 'uint16':    
                    ...
                    imgdataFF = np.flipud(np.reshape(imgdata, (self.numrowsFF, self.numcolsFF), order='A'))
                    self.nxinstrument['bright_field']['data'] = nxs.NXfield(
                        name='data', value = imgdataFF, dtype=self.datatypeFF , shape=[self.numrowsFF, self.numcolsFF])
                    self.nxinstrument['bright_field']['data'].attrs['Data Type']=self.datatypeFF 
                    self.nxinstrument['bright_field']['data'].attrs['Number of images']=self.nSampleFramesFF
                    self.nxinstrument['bright_field']['data'].attrs['Image Height']=self.numrowsFF
                    self.nxinstrument['bright_field']['data'].attrs['Image Width']=self.numcolsFF
                    ...
                    imgdataFF = np.flipud(np.reshape(imgdata, (self.numrowsFF, self.numcolsFF), order='A'))
                    self.nxinstrument['bright_field']['data'] = nxs.NXfield(
                        name='data', value = imgdataFF, dtype='float' , shape=[self.numrowsFF, self.numcolsFF])
                    self.nxinstrument['bright_field']['data'].attrs['Data Type']=self.datatypeFF 
                    self.nxinstrument['bright_field']['data'].attrs['Number of images']=self.nSampleFramesFF
                    self.nxinstrument['bright_field']['data'].attrs['Image Height']=self.numrowsFF
                    self.nxinstrument['bright_field']['data'].attrs['Image Width']=self.numcolsFF
            ...
            self.nxdata['data'] = nxs.NXlink(target=self.nxinstrument['bright_field']['data'], group=self.nxdata)
            self.nxdata['data'].write()
        
            oleFF.close()    
            print ("Mosaic FF image data conversion to NeXus HDF5 has been done.\n")
            return
