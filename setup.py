...
which allows converts .xrm and .txrm image stack files (output of the 
TXM Microscope at BL09-Mistral), into HDF5 files (txrm2nexus and xrm2nexus).

Mosaics generated by the TXM can also be converted (mosaic2nexus).
A script to normalize images using the FF and a ratio number 
(ratio between acquisition time of Tomo and of FF) is also provided (normalize).
...
setup(
    ...
    author='Marc Rosanes, Carlos Falcon',
    author_email='mrosanes@cells.es, cfalcon@cells.es',
    ...
    # It also depends of Nexus nxs, but it is not added here because nxs is not
