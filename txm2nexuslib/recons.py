...
class recons_normalize:
    ...
    def normalize_tomo(self):
        ...
        try: 
            ...
            self.input_nexusfile.closedata()
            self.tomonorm['x_pixel_size'] = self.x_pixel_size
            self.tomonorm['x_pixel_size'].write()
            self.tomonorm['y_pixel_size'] = self.y_pixel_size
            self.tomonorm['y_pixel_size'].write()    
        except:
        ...
        if (self.boolean_current_exists == 1):
            ...
            if (dimensions_singleimage_tomo==dimensions_singleimage_brightfield):     
                ...
                if self.avgff == 0:
                    print('\nFFs have been calculated using the machine_currents\n')  
                    self.averagebright = np.array(self.data_brightfield[0]/self.ratios_currents_brightfield[0], dtype = np.float)
                ...
                if self.avgff == 1:
                    ...
                    if(self.gaussianblur != 0):
                        ...
                        self.averagebright = ndimage.gaussian_filter(
                                       self.averagebright, sigma=self.gaussianblur)
